...
class StellarSimTDep:
    ...
    def _sort_modes_for_sphht(self, aj):
        ...
        aj = jnp.asarray(aj)
        aj_sorted = jnp.concatenate(
            [aj[perm], jnp.zeros(K, dtype=aj.dtype)])
        parent_j_sorted = jnp.concatenate(
            [self.parent_j[perm], jnp.zeros(K, dtype=self.parent_j.dtype)])
        return aj_sorted, parent_j_sorted
    ...
    def compute_rho_lm_at_particles_sphht(self, R_j_at_parts, phase_c, sparse_au_j):
        ...
        *bound-method* jit, so anything reached through `self` becomes a
        ...
        return MSS.compute_rho_lm_at_particles_sphht_jit(
            ...
            aj_sorted, parent_j_sorted, blocks,
