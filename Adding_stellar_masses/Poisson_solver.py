...
def compute_phi_lm_and_deriv(rho_lms, rho_lm_at_particle, gather_idx, insert_mask,
    ...
    # r_updated has all particle positions inside it
    ...
    ratio = jnp.clip(r_updated / r_ref, 0.0, 1.0)      
                  
    r_fold_int = jnp.power(ratio[None, :], (2 + ell_f[:, None]))  # (L_max, Nr+1)
    r_fold_ext = jnp.power(ratio[None, :], (1 - ell_f[:, None]))          # (L_max, Nr+1)
    ...
    def chunk_compute(args):
        ...
        rho_band     = rho_lms[:, l_vals, m_inds]         # (Nr, l_band_size)
        f_orig_band  = rho_band[gather_idx].T             # (l_band_size, Nr+1)
        f_ins_band   = rho_lm_at_particles[l_vals, m_inds]  # (l_band_size,)
        f_at_lm_band = jnp.where(insert_mask[None, :],
                                    f_ins_band[:, None],
                                    f_orig_band)            # (l_band_size, Nr+1)
