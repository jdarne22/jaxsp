...
class StellarSimTDep:
    ...
    def construct_acc_master_func(self, positions_sph, eval_library, eigenstate_lib, poten = False):
        ...
        if self.plot:
            ...
            if self.SphHT:
                _rho_p0 = self.compute_rho_lm_at_particles_sphht(_R_j_p0, phase_c)
            else:
    ...
    def run_simulation(self, checkpoint_every=50, checkpoint_dir=None):
        ...
        if checkpoint_dir is None:
            ...
            checkpoint_dir = os.path.join(
                _script_dir, "Checkpoints_test",
                f"checkpoints_test_m22_{self.m22:g}_r0_{self.r_half:g}_frozen{int(self.frozen)}_static{int(self.static)}"
            )
