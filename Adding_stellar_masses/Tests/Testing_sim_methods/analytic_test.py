...
class StellarSimTDep:
    ...
    def construct_frozen_rho(self, aj, Y_lm, parent_j, eigen_energies):
    ...
    def construct_acc(self, r_pos_sph, lm_pairs, density_params, flm_r, eval_library, eigenstate_lib):
        ...
        if self.frozen:
            ...
            R_j_at_particle = eval_library(jnp.array([particle_r]), eigenstate_lib.radial_eigenmode_params)[0]# New bin phased radial wavefunction

            psi_at_r   = jnp.einsum('j,jtp->tp', R_j_at_particle, self.Q_j_tp)  # (n_theta, n_phi) Q_j_tp is a_j * Y_lm
            rho_rtp   = self.total_mass * jnp.abs(psi_at_r) ** 2               # (n_theta, n_phi)
    ...
    def run_simulation(self):
        ...
        if self.static == True:
            ...
            # 1. Construct total psi and rho on background grid
            start = time()
            rho_rtp = self.construct_frozen_rho(aj, Y_lm, parent_j, eigen_energies)
            end = time()
            print(f"Constructing total psi and rho completed in {end - start:.2f} seconds")
