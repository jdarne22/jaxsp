...
for ckpt_dir, L_frac in zip(dirs, L_out_fracs):
    ...
    R_median.append(np.median(r_vals, axis=0))
...
for i, L_frac in enumerate(L_out_fracs):
    ...
    ax.plot(x, R_median[i] * to_Kpc, label=f"L_out_frac={L_frac}",
            color=line_colors[i], alpha=0.9)
...
out1 = os.path.join(PLOT_DIR, f"L_reduc_all_m22_{m22_str}_r0_{r0_str}_median.png")
